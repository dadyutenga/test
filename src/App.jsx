  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-twilight via-dusk to-black text-white">
      <header className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-20 flex flex-col gap-10 sm:gap-12 items-center text-center">
          <motion.h1
            className="font-display text-3xl sm:text-5xl lg:text-6xl tracking-wide"
          ...
          <motion.p
            className="max-w-3xl text-base sm:text-xl text-slate-200"
          ...
          <motion.div
            className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur p-6 sm:p-8 shadow-xl max-w-3xl space-y-4 text-left sm:text-center"
          ...
          <motion.div
            className="grid w-full max-w-4xl grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6"
            initial="hidden"
            whileInView="visible"
            ...
            {gestures.slice(0, 2).map((gesture, idx) => {
              ...
              return (
                <motion.div
                  ...
                  custom={idx}
                  className="rounded-2xl border border-white/10 bg-black/30 backdrop-blur p-4 sm:p-5 flex flex-col items-center gap-3"
                >
                  <Icon className="w-12 h-12 text-blush drop-shadow-[0_0_12px_rgba(244,114,182,0.6)]" />
                  <h3 className="font-semibold text-base sm:text-lg">{gesture.title}</h3>
                  <p className="text-slate-300 text-sm sm:text-base text-center leading-relaxed">{gesture.detail}</p>
                </motion.div>
              );
            })}
          </motion.div>
      ...
      <main className="relative space-y-16 sm:space-y-24 pb-20 sm:pb-24">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {gestures.map((gesture) => {
              ...
              return (
                <motion.div
                  key={gesture.id}
                  className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-lg p-5 sm:p-6 flex flex-col gap-3"
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                  ...
                  <p className="text-sm sm:text-base text-slate-300 leading-relaxed">{gesture.detail}</p>
                </motion.div>
              );
            })}
          </div>
          ...
          <section key={beat.id} className="relative">
            ...
            <div className="relative max-w-6xl mx-auto px-4 sm:px-6 grid lg:grid-cols-2 gap-10 lg:gap-12 items-center">
              ...
              >
                ...
                <motion.h2
                  className="font-display text-2xl sm:text-4xl text-white"
                  variants={textVariants}
                  custom={1}
                >
                  {beat.title}
                </motion.h2>
                <motion.p className="text-base sm:text-lg text-slate-200 leading-relaxed" variants={textVariants} custom={2}>
                  {beat.subtitle}
                </motion.p>
                <motion.blockquote className="text-base sm:text-lg text-blush/90 italic" variants={textVariants} custom={3}>
                  “{beat.swahili}”
                </motion.blockquote>
                <motion.p className="text-sm sm:text-base leading-relaxed text-slate-300 whitespace-pre-line" variants={textVariants} custom={4}>
                  {beat.body}
                </motion.p>
              </motion.div>
              <motion.div
              ...
              >
                <div className="absolute -inset-3 sm:-inset-6 rounded-3xl bg-white/5 blur-3xl" aria-hidden />
      ...
      <footer className="px-4 sm:px-6 py-12 sm:py-16 text-center text-slate-400 text-sm leading-relaxed">
